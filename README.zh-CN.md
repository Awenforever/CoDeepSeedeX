...
|---|---|---|---|---|
| v0.3.9-alpha / p2.10a50 | WeClaw完整telemetry与Release readiness | WeClaw联动此前只能把usage、pricing、cost、balance和辅助调用细节视为不可用或不完整。 | dsproxy现在提供WeClaw完整telemetry基线，包括usage聚合、pricing元数据、estimated cost、provider balance、辅助模型调用统计和明确精确性边界。 | 如果使用WeClaw联动，要求`weclaw_dev >= v0.1.9-alpha`。可用`dsproxy upgrade --alpha`安装该pre-release。 |
