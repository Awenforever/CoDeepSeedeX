...
| v0.3.8-alpha / p2.10a26 | Codex wrapper启动和Plan mode加固 | proxy路由不可用时，Codex仍可能进入TUI，原生Plan mode仍可能暴露`medium`。 | wrapper现在会先启动并验证对应proxy路由，再进入Codex。生成的DeepSeek profile包含`plan_mode_reasoning_effort = "high"`，uninstall会在存在备份时恢复旧Codex命令。 | 安装`v0.3.8-alpha`pre-release，或等待其提升为Latest。 |
| v0.3.8-alpha / p2.10a25 | 安装器版本和非git fallback体验 | source archive安装可能丢失目标commit，已有非git安装目录会先打印git clone fatal，pip输出也会污染TTY。 | 安装器现在持久化目标commit，已有非git安装目录直接进入source archive fallback，pip输出写入install log。 | 安装`v0.3.8-alpha`pre-release，或等待其提升为Latest。 |


| v0.3.8-alpha / p2.10a24 | 安装器输出优化与图像live验证 | Bootstrap在安装器UI前单独打印log行，image API验证仍使用脆弱的非生成式探针。 | Bootstrap log和install log统一放到`Install logs`区块。Image API配置会提示验证将生成一张安全测试图并可能消耗额度，然后执行真实图像生成验证并把测试图保存到`/tmp`下。 | 已包含在`v0.3.8-alpha`中；安装pre-release，或等待其提升为Latest。 |


| v0.3.8-alpha / p2.10a23 | 安装器图像验证与移动pre-release tag获取 | 引导式image API流程调用了未定义的`test_image_api_key`函数，导致VM真实安装时运行期失败。反复重建pre-release tag后，`git fetch --tags`也可能因`would clobber existing tag`失败。 | 补齐缺失的非生成式image API验证函数，新增安装器测试以确认项目内shell调用均已定义且先定义后使用，并将安装器tag获取改为`git fetch --tags --force origin`以支持pre-release循环测试。 | 已包含在`v0.3.8-alpha`中；安装pre-release，或等待其提升为Latest。 |


| v0.3.8-alpha / p2.10a22 | 安装器端口标签和effort表面 | 安装器仍使用`Stable proxy port`，CLI升级重装profile路径仍可能为non-thinking Codex profile写入`medium`。 | 安装器改为`Non-Thinking proxy port`。CoDeepSeedeX自己安装和升级写入的profile只使用`high`或`xhigh`，proxy仍兼容旧`low`和`medium`输入并归一化为DeepSeek侧`high`。 | 已包含在`v0.3.8-alpha`中；安装pre-release，或等待其提升为Latest。 |
| v0.3.8-alpha / p2.10a21 | 安装器wrapper说明位置调整 | Codex wrapper说明出现在问题上方，和对应问题视觉上分离。 | wrapper说明现在由菜单渲染器打印在问题下方，并位于全局方向键提示之前。 | 已包含在`v0.3.8-alpha`中；安装pre-release，或等待其提升为Latest。 |
| v0.3.8-alpha / p2.10a20 | 安装器密钥输入提示与wrapper说明 | 已有model API key时直接按Enter会复用隐藏默认值，并把它误报为新输入字符数。密钥输入辅助提示和wrapper用途说明不够清晰。 | 密钥输入辅助信息改为淡色。有已有key时空输入只保留原key，不再计数字符或重新验证。安装器现在说明Codex wrapper安装后可用`codex --profile deepseek`和`codex --profile deepseek-thinking`，并自动处理本地dsproxy后台。 | 已包含在`v0.3.8-alpha`中；安装pre-release，或等待其提升为Latest。 |
| v0.3.8-alpha / p2.10a19 | 安装器菜单选中行列对齐 | 选中行使用`▶ `，未选中行使用更宽空白前缀，导致选中项视觉上左移。 | 未选中行改用两个空格前缀，使选中与未选中编号列对齐。 | 已包含在`v0.3.8-alpha`中；安装pre-release，或等待其提升为Latest。 |
| v0.3.8-alpha / p2.10a18 | 极简方向键安装器UI | p2.10a17安装器意外保留了重复旧菜单函数，导致VM运行时旧的数字/text fallback渲染器覆盖新渲染器。 | 删除重复菜单定义。菜单只保留↑/↓或j/k、Enter和Backspace。帮助提示改为淡色且只显示一次。端口默认值提示改为淡色。Bootstrap不再打印重复Python/Installer ready行。 | 已包含在`v0.3.8-alpha`中；安装pre-release，或等待其提升为Latest。 |
| v0.3.8-alpha / p2.10a17 | 安装器菜单渲染与排版优化 | 方向键菜单在长选项自动换行时会残留重复行，数字快捷键`0`不是直接选择，相邻配置提示也容易粘连。 | 菜单行现在按终端宽度截断，选中项使用整行反色视频高亮，列表数字可直接选择，帮助提示只显示一次，各配置块之间加入空行。 | 已包含在`v0.3.8-alpha`中；安装pre-release，或等待其提升为Latest。 |
| v0.3.8-alpha / p2.10a16 | 安装器Logo heredoc运行时修复 | p2.10a15将ASCII Logo放入未引用heredoc，Logo中的反引号在运行时触发shell命令替换。 | Logo图案改用引用heredoc，版本行单独打印，并新增运行时Logo烟测，避免`bash -n`漏检此类问题。 | 已包含在`v0.3.8-alpha`中；安装pre-release，或等待其提升为Latest。 |
| v0.3.8-alpha / p2.10a15 | 安装器provider分层流程与源码归档fallback | provider菜单过平，Yes/No误显示Supported，Logo缺少版本号，且Release资产已下载后仍可能因`git fetch --tags`失败中断VM安装。 | 安装器现在按provider族再进入endpoint/region子菜单，Yes/No无状态标记，Logo显示install ref，密钥输入提示字符数，git setup失败时可fallback到指定tag源码归档。 | 已包含在`v0.3.8-alpha`中；安装pre-release，或等待其提升为Latest。 |
| v0.3.8-alpha / p2.10a14 | 安装器来源日志变量修复 | p2.10a13把来源详情从界面移到日志时写入了未定义的`LOG_FILE`。 | 来源详情现在写入`INSTALL_LOG`，并通过测试断言安装器不再引用`LOG_FILE`。 | 已包含在`v0.3.8-alpha`中；安装pre-release，或等待其提升为Latest。 |
| v0.3.8-alpha / p2.10a13 | 安装器UI压缩与TTY菜单路由 | Bootstrap/install界面打印完整来源URL，且stdout被日志捕获时方向键菜单会fallback为普通输入。 | 交互界面只保留Logo旁版本号，来源详情写入日志，方向键菜单通过`/dev/tty`渲染和读取。 | 已包含在`v0.3.8-alpha`中；安装pre-release，或等待其提升为Latest。 |
| v0.3.8-alpha / p2.10a12 | Bootstrap install-ref与Release资产解析 | `bootstrap.sh --install-ref v0.3.8-alpha`仍会优先下载Latest的`install.sh`，导致pre-release全新VM测试进入旧安装器。 | Bootstrap现在会消费`--install-ref`，优先下载对应Release资产中的`install.sh`，并在banner下显示bootstrap/installer来源。 | 已包含在`v0.3.8-alpha`中；安装pre-release，或等待其提升为Latest。 |
