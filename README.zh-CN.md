...
当前`v0.3.9-alpha` Release包含累计WeClaw侧状态契约，并新增p2.12稳定线：Codex profile前向兼容修复、只认比例的auto-compact策略、token-only Compact/Trim状态、持久化runtime payload report，以及已通过真实路由验证的低风险semantic payload compaction。累计Release notes见`docs/release-notes-v0.3.9-alpha.md`。
