...
安装器会把CoDeepSeedeX放到`~/.local/share/deepseek-responses-proxy`，创建`dsproxy`命令，创建主受管Codex profile：`deepseek-thinking`，并可安装一个窄作用域`codex`wrapper，用于CoDeepSeedeX受管profile和provider-backed custom profiles。安装器还会写入shell启动片段，使新终端优先命中`~/.local/bin/dsproxy`和`~/.local/bin/codex`。
...
在引导式wizard中添加custom provider时，provider name会经过slug归一化后成为本地provider id/profile id。CoDeepSeedeX不声明接管Codex原生`/model`列表；模型切换请使用`dsproxy provider add-model/use`或provider-backed profile。

本节点不声明已接入Codex TUI原生`/model`列表；在实测Codex确实读取profile级model catalog前，请使用`dsproxy provider add-model/use`或provider-backed profiles切换模型。
