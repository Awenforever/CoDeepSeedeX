...
显式pre-release通道，当前为`v0.3.9-alpha`：
...
如果WeClaw联动使用CoDeepSeedeX `v0.3.9-alpha`，WeClaw版本必须不低于：
...
- 通过`runtime_payload_guard`暴露的char级Compact和Trim fallback/debug进度；
...
当前`v0.3.9-alpha` Release已包含最新WeClaw侧状态契约：当前session token/cost作用域、auxiliary token零对象、session级Details、prompt reconciliation、Details来源拆分、人民币优先的Pricing/Cost字段、Compact/Trim信息保有率字段，以及同版本commit感知的upgrade行为。累计Release notes见`docs/release-notes-v0.3.9-alpha.md`。
