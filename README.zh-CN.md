...
|---|---|---|---|---|
| v0.3.9-alpha / p2.10a50 | WeClaw完整telemetry与Release readiness | WeClaw联动此前只能把usage、pricing、cost、balance和辅助调用细节视为不可用或不完整。 | dsproxy现在提供WeClaw完整telemetry基线，包括usage聚合、pricing元数据、estimated cost、provider balance、辅助模型调用统计和明确精确性边界。 | 如果使用WeClaw联动，要求`weclaw_dev >= v0.1.9-alpha`。发布后可用`dsproxy upgrade --alpha`安装该pre-release。 |
| v0.3.9-alpha / p2.10a43-p2.10a49 | 集成安全profile与文档控制 | profile/effort自动化和跨对话任务规划容易在插入任务后漂移。 | 新增effort工作流JSON/refresh控制，完成WeClaw已认可契约基线，并记录长期主线任务表和完整源码优先开发规则。 | 现有用户可保持正常配置。集成客户端应优先使用文档中的`profile`和`status --weclaw-json`契约。 |
| unreleased / p2.10a36 | Release状态文档同步 | GitHub Release已是当前非pre-release Release后，文档仍把`v0.3.8-alpha`描述为pre-release或等待提升为Latest。 | 同步README和开发手册中的当前Release状态表述，同时保留alpha tag命名策略和历史pre-release验证规则。 | 当前`v0.3.8-alpha`走普通安装或升级路径。 |
| unreleased / p2.10a35 | 文档移交和整体替换纪律 | 手册仍残留p2.10a26旧状态，并分散记录了已被后续推翻的标题刷新实验。反复使用狭窄字符串锚点也造成了可避免的测试补丁失败。 | 更新开发手册和development-log，压缩p2.10a30到p2.10a34的wrapper经验为当前p2.10a34有效设计，并记录函数级、块级、章节级和AST级整体替换规则。 | 新对话以开发手册作为启动上下文。 |
| unreleased / p2.10a34 | Codex返回后清理有限tab标题keeper | 有限标题keeper可能在Codex返回后继续运行到60秒上限，导致用户离开Codex后标题又短暂恢复为CoDeepSeedeX。 | wrapper现在记录keeper PID，并在真实Codex命令返回后停止并等待keeper结束，同时保留真实Codex返回状态。 | 用`dsproxy profile refresh-wrapper --json`刷新本地wrapper。 |
| unreleased / p2.10a33 | 有限运行期tab标题keeper | 延迟标题刷新后台任务把stdout重定向到`/dev/null`后，标题函数因先检查stdout TTY而直接跳过`/dev/tty`写入。Codex也可能在单次延迟OSC后再次改标题。 | wrapper现在允许stdout不是TTY但`/dev/tty`可写时继续发OSC，并默认运行60秒有限keeper，每1秒刷新一次标题。 | 用`dsproxy profile refresh-wrapper --json`刷新本地wrapper。 |
| unreleased / p2.10a32 | 前台Codex wrapper确保延迟标题可靠 | wrapper用真实Codex进程替换自身时，延迟OSC标题刷新仍不可靠。 | wrapper现在保持自身存活，并以前台命令运行真实Codex，使有限次数的延迟OSC刷新能在Codex启动阶段执行，同时自然保留真实Codex返回状态。 | 用`dsproxy profile refresh-wrapper --json`刷新本地wrapper。 |
| unreleased / p2.10a31 | Codex启动后tab标题刷新 | wrapper此前在Codex启动前设置tab标题，因此Codex仍可能用工作目录标题覆盖它。 | wrapper现在先等待对应dsproxy route就绪，然后只在启动后安排延迟OSC 0/2刷新，不再在Codex开始启动前改标题。 | 用`dsproxy profile refresh-wrapper --json`刷新本地wrapper。 |
| unreleased / p2.10a30 | Profile model同步和延迟tab标题 | 即使dsproxy实际路由到另一个有效上游模型，Codex TUI仍可能显示旧Codex profile model。Codex启动时也会覆盖wrapper预先设置的tab标题。 | 新增`dsproxy profile repair --managed-only --json`，把managed profile的`model`字段同步到各profile自己的有效上游模型。Codex wrapper现在会在启动后进行短时延迟OSC标题刷新，覆盖Codex启动期标题更新。 | 安装该内部构建后执行`dsproxy profile repair --managed-only --json`和`dsproxy profile refresh-wrapper --json`。 |
| unreleased / p2.10a29 | WeClaw运行时契约统一 | WeClaw-facing status仍只是CLI骨架，HTTP WeClaw端点缺失，已安装Codex wrapper可能停留在旧版本，model/context字段会混合Codex profile声明和dsproxy运行时行为。 | dsproxy现在明确输出effective model字段、wrapper刷新入口、HTTP WeClaw status/profile端点，并用context契约区分Codex profile token级窗口和runtime char级compaction/trimming。 | WeClaw应优先展示`effective_model`，检查`model_conflict`，并从WeClaw契约读取context，不再解析私有文件。 |
| unreleased / p2.10a28 | CoDeepSeedeX / WeClaw profile契约 | WeClaw需要推断或修补profile绑定状态，且`set-effort max`可能写出Codex不接受的`model_reasoning_effort = "max"`。 | dsproxy统一维护profile契约：DeepSeek env侧保存`max`，Codex profile侧保存`xhigh`，`profile status --json`输出profile健康状态，`status --weclaw-json`输出WeClaw可消费的稳定骨架。 | 安装该内部构建后，重新执行`dsproxy config set-effort max`或`dsproxy profile set-effort deepseek-thinking max --json`。 |
| v0.3.8-alpha / p2.10a26 | Codex wrapper启动和Plan mode加固 | proxy路由不可用时，Codex仍可能进入TUI，原生Plan mode仍可能暴露`medium`。 | wrapper现在会先启动并验证对应proxy路由，再进入Codex。生成的DeepSeek profile包含`plan_mode_reasoning_effort = "high"`，uninstall会在存在备份时恢复旧Codex命令。 | 正常安装或升级到当前GitHub Release `v0.3.8-alpha`。 |
| v0.3.8-alpha / p2.10a25 | 安装器版本和非git fallback体验 | source archive安装可能丢失目标commit，已有非git安装目录会先打印git clone fatal，pip输出也会污染TTY。 | 安装器现在持久化目标commit，已有非git安装目录直接进入source archive fallback，pip输出写入install log。 | 正常安装或升级到当前GitHub Release `v0.3.8-alpha`。 |


| v0.3.8-alpha / p2.10a24 | 安装器输出优化与图像live验证 | Bootstrap在安装器UI前单独打印log行，image API验证仍使用脆弱的非生成式探针。 | Bootstrap log和install log统一放到`Install logs`区块。Image API配置会提示验证将生成一张安全测试图并可能消耗额度，然后执行真实图像生成验证并把测试图保存到`/tmp`下。 | 已包含在当前GitHub Release `v0.3.8-alpha`中；正常安装或升级即可。 |


| v0.3.8-alpha / p2.10a23 | 安装器图像验证与移动pre-release tag获取 | 引导式image API流程调用了未定义的`test_image_api_key`函数，导致VM真实安装时运行期失败。反复重建pre-release tag后，`git fetch --tags`也可能因`would clobber existing tag`失败。 | 补齐缺失的非生成式image API验证函数，新增安装器测试以确认项目内shell调用均已定义且先定义后使用，并将安装器tag获取改为`git fetch --tags --force origin`以支持pre-release循环测试。 | 已包含在当前GitHub Release `v0.3.8-alpha`中；正常安装或升级即可。 |


| v0.3.8-alpha / p2.10a22 | 安装器端口标签和effort表面 | 安装器仍使用`Stable proxy port`，CLI升级重装profile路径仍可能为non-thinking Codex profile写入`medium`。 | 安装器改为`Non-Thinking proxy port`。CoDeepSeedeX自己安装和升级写入的profile只使用`high`或`xhigh`，proxy仍兼容旧`low`和`medium`输入并归一化为DeepSeek侧`high`。 | 已包含在当前GitHub Release `v0.3.8-alpha`中；正常安装或升级即可。 |
| v0.3.8-alpha / p2.10a21 | 安装器wrapper说明位置调整 | Codex wrapper说明出现在问题上方，和对应问题视觉上分离。 | wrapper说明现在由菜单渲染器打印在问题下方，并位于全局方向键提示之前。 | 已包含在当前GitHub Release `v0.3.8-alpha`中；正常安装或升级即可。 |
| v0.3.8-alpha / p2.10a20 | 安装器密钥输入提示与wrapper说明 | 已有model API key时直接按Enter会复用隐藏默认值，并把它误报为新输入字符数。密钥输入辅助提示和wrapper用途说明不够清晰。 | 密钥输入辅助信息改为淡色。有已有key时空输入只保留原key，不再计数字符或重新验证。安装器现在说明Codex wrapper安装后可用`codex --profile deepseek`和`codex --profile deepseek-thinking`，并自动处理本地dsproxy后台。 | 已包含在当前GitHub Release `v0.3.8-alpha`中；正常安装或升级即可。 |
| v0.3.8-alpha / p2.10a19 | 安装器菜单选中行列对齐 | 选中行使用`▶ `，未选中行使用更宽空白前缀，导致选中项视觉上左移。 | 未选中行改用两个空格前缀，使选中与未选中编号列对齐。 | 已包含在当前GitHub Release `v0.3.8-alpha`中；正常安装或升级即可。 |
| v0.3.8-alpha / p2.10a18 | 极简方向键安装器UI | p2.10a17安装器意外保留了重复旧菜单函数，导致VM运行时旧的数字/text fallback渲染器覆盖新渲染器。 | 删除重复菜单定义。菜单只保留↑/↓或j/k、Enter和Backspace。帮助提示改为淡色且只显示一次。端口默认值提示改为淡色。Bootstrap不再打印重复Python/Installer ready行。 | 已包含在当前GitHub Release `v0.3.8-alpha`中；正常安装或升级即可。 |
| v0.3.8-alpha / p2.10a17 | 安装器菜单渲染与排版优化 | 方向键菜单在长选项自动换行时会残留重复行，数字快捷键`0`不是直接选择，相邻配置提示也容易粘连。 | 菜单行现在按终端宽度截断，选中项使用整行反色视频高亮，列表数字可直接选择，帮助提示只显示一次，各配置块之间加入空行。 | 已包含在当前GitHub Release `v0.3.8-alpha`中；正常安装或升级即可。 |
| v0.3.8-alpha / p2.10a16 | 安装器Logo heredoc运行时修复 | p2.10a15将ASCII Logo放入未引用heredoc，Logo中的反引号在运行时触发shell命令替换。 | Logo图案改用引用heredoc，版本行单独打印，并新增运行时Logo烟测，避免`bash -n`漏检此类问题。 | 已包含在当前GitHub Release `v0.3.8-alpha`中；正常安装或升级即可。 |
| v0.3.8-alpha / p2.10a15 | 安装器provider分层流程与源码归档fallback | provider菜单过平，Yes/No误显示Supported，Logo缺少版本号，且Release资产已下载后仍可能因`git fetch --tags`失败中断VM安装。 | 安装器现在按provider族再进入endpoint/region子菜单，Yes/No无状态标记，Logo显示install ref，密钥输入提示字符数，git setup失败时可fallback到指定tag源码归档。 | 已包含在当前GitHub Release `v0.3.8-alpha`中；正常安装或升级即可。 |
| v0.3.8-alpha / p2.10a14 | 安装器来源日志变量修复 | p2.10a13把来源详情从界面移到日志时写入了未定义的`LOG_FILE`。 | 来源详情现在写入`INSTALL_LOG`，并通过测试断言安装器不再引用`LOG_FILE`。 | 已包含在当前GitHub Release `v0.3.8-alpha`中；正常安装或升级即可。 |
| v0.3.8-alpha / p2.10a13 | 安装器UI压缩与TTY菜单路由 | Bootstrap/install界面打印完整来源URL，且stdout被日志捕获时方向键菜单会fallback为普通输入。 | 交互界面只保留Logo旁版本号，来源详情写入日志，方向键菜单通过`/dev/tty`渲染和读取。 | 已包含在当前GitHub Release `v0.3.8-alpha`中；正常安装或升级即可。 |
| v0.3.8-alpha / p2.10a12 | Bootstrap install-ref与Release资产解析 | `bootstrap.sh --install-ref v0.3.8-alpha`仍会优先下载Latest的`install.sh`，导致pre-release全新VM测试进入旧安装器。 | Bootstrap现在会消费`--install-ref`，优先下载对应Release资产中的`install.sh`，并在banner下显示bootstrap/installer来源。 | 已包含在当前GitHub Release `v0.3.8-alpha`中；正常安装或升级即可。 |
| v0.3.8-alpha / p2.10a11 | 模型provider支持级别标记 | 非DeepSeek模型provider此前标为Supported，但目前只验证过API连通性。 | 只有DeepSeek保留Supported。Kimi、Zhipu / BigModel、Z.AI和Qwen / DashScope模型provider改为Experimental，直到完整Codex工作流验证通过。 | 这是支持级别分类和交互提示修正。 |
| v0.3.8-alpha / p2.10a10 | 安装器provider选择交互 | 安装器provider菜单主要依赖数字输入，部分image provider提示仍使用泛化Qwen名称。 | 引导式安装菜单优先使用方向键选择，并保留数字/文本fallback。image provider提示改为显式Qwen地区名称。 | 仅修改安装器交互体验，不移动Release tag。 |
| v0.3.8-alpha / p2.10a8 | Alpha升级通道和Codex选项卡标题 | `dsproxy upgrade`只跟随GitHub Latest Release，Codex wrapper不会设置终端选项卡标题。 | `dsproxy upgrade --alpha`现在跟随最新的非draft GitHub pre-release，普通`dsproxy upgrade`仍跟随Latest Release。Codex wrapper会在`deepseek`和`deepseek-thinking`profile启动时随机设置`[emoji]CoDeepSeedeX`格式的tab标题。 | 历史说明：`dsproxy upgrade --alpha`曾是提升为Latest前的VM验证路径。当前Release走普通安装或升级路径。 |
