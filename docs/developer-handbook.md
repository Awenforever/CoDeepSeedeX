...
- Current public Release: `v0.4.6-alpha`
...
- GitHub Latest ordinary Release: `v0.4.6-alpha`
- GitHub Release title: `CodeXchange v0.4.6-alpha`
...
- Public tag `v0.4.6-alpha = <pending>`.
...
- GitHub Latest API returns `v0.4.6-alpha`.
- Release assets are exactly `bootstrap.sh` and `install.sh`.
- `cox --version` from the refreshed release line reports `public version: v0.4.6-alpha | b11a1c4`.
...
- This documentation sync may advance `master` beyond the public Release commit. The public `v0.4.6-alpha` tag is being refreshed by this explicit Release-update task to the p2.22 closeout release commit.
...
`v0.4.6-alpha` currently covers these user-visible areas:
...
| --- | --- | --- | --- | --- | --- | --- |
| Release | `v0.4.6-alpha` current Latest | GitHub Latest Release exists with `isPrerelease=false`, assets exactly `bootstrap.sh` and `install.sh`, and no duplicate Release title in body. | `v0.4.6-alpha = <pending>` | Closed | 2026-06-08 | Includes the p2.19 custom provider registry, guided UI, Codex compatibility/wrapper hardening, status JSON, auxiliary-model leakage fix, profile-drift fail-closed guard, and real Codex entry validation. |
