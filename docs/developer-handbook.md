...
- Current public Release commit: `01d6cee`
...
- VM real-home validation passed for custom provider setup, Codex wrapper execution, legacy Codex profile layout, context-window profile-source consistency, and guided-hint contamination checks.

## 3. Key file map

- `deepseek_responses_proxy/app.py`: runtime core, Responses-compatible API, DeepSeek/custom provider bridge, tool bridge, provider dispatch, version metadata, debug trace.
- `deepseek_responses_proxy/cli.py`: `dsproxy` CLI, configuration, provider setup, custom provider registry, post-config refresh, doctor commands, upgrade logic.
...
1. Guided installer and `dsproxy config wizard`
...
3. Codex compatibility and wrapper reliability
   ...
   - The generated Codex wrapper resolves the real Codex binary and must not point `REAL_CODEX` at another CoDeepSeedeX wrapper or a `/tmp/codeepseedex-*` test wrapper.
...
4. Status and diagnostics
   - `dsproxy profile status` reports profile source and profile layout consistently.
   ...
   - `dsproxy config show` and `dsproxy config test-api-key` expose validation method and URL without logging API-key material.

5. Provider bridges
   - Web search tool bridge
   - Image generation tool bridge
   - Web search validation can consume quota.
   - Image generation default validation may be non-generating; real image generation requires explicit `dsproxy doctor providers --live --allow-spend`.
...
12. **AnyCodeX future-name boundary.** CoDeepSeedeX remains the current project and public product name. AnyCodeX is a future plan name, not a current code, command, tag, branch, installer, wrapper, public path, or user-facing documentation name.
...
Any development task that can affect an installed user environment must include install, upgrade, and uninstall in the design review. At minimum, check one-line bootstrap, `scripts/install.sh`, `dsproxy upgrade`, generated `dsproxy` or `codex` wrappers, `~/.codex/config.toml`, local env files, manifest-backed rollback, source archive fallback, and Release assets.
...
- Runtime compatibility is preserved through the legacy env mirror: `DEEPSEEK_PROXY_MODEL_PROVIDER=custom`, `DEEPSEEK_BASE_URL`, `DEEPSEEK_PROXY_MODEL`, and `DEEPSEEK_API_KEY`.
...
- Codex CLI `< 0.134.0`: use legacy `[profiles.deepseek]` and `[profiles.deepseek-thinking]` tables in `~/.codex/config.toml`.
- Codex CLI `>= 0.134.0`: use split profile files `~/.codex/deepseek.config.toml` and `~/.codex/deepseek-thinking.config.toml`.

The wrapper contract:

- `REAL_CODEX` must resolve to the actual Codex binary or Node entry, not another CoDeepSeedeX wrapper.
- Paths under `/tmp/codeepseedex-*` must not be selected as real Codex.
...
Uninstall must not delete unrelated user files or non-CoDeepSeedeX configuration.
...
| --- | --- | --- | --- | --- | --- | --- |
| Release | `v0.4.3-alpha` current Latest | GitHub Latest Release exists with `isPrerelease=false`, assets exactly `bootstrap.sh` and `install.sh`, and no duplicate Release title in body. | `v0.4.3-alpha = 01d6cee` | Closed | 2026-06-07 | Includes p2.19 custom provider registry, guided UI, Codex legacy/split profile compatibility, wrapper real-binary fix, and contextual hints. |
...
| WeClaw | Full telemetry baseline | WeClaw can consume dsproxy-owned status contracts for profile, model, effort, context, usage, pricing, cost, balance, Details, tokenizer, and compaction. | `v0.3.9-alpha = 82a4428` | Closed | 2026-05-24 | Requires `weclaw_dev >= v0.1.9-alpha` if used. |
...
- A generated CoDeepSeedeX Codex wrapper must never use another CoDeepSeedeX wrapper as `REAL_CODEX`.
