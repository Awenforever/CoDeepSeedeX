...
- Current public Release: `v0.4.41-alpha`
- Current public Release kind: ordinary GitHub Latest alpha Release, with `isPrerelease=false`
- Current public Release commit: `release v0.4.41-alpha`
- GitHub Latest ordinary Release: `v0.4.41-alpha`
- GitHub Release title: `CodeXchange v0.4.41-alpha`
...
- Current internal development checkpoint: `p3.3a20a51-provider-pricing-daily-refresh-explicit-runtime-entry-wiring-v0443`
- Latest runtime checkpoint included in the public Release: `p3.3a20a51-provider-pricing-daily-refresh-explicit-runtime-entry-wiring-v0443`
...
- Current public Release note synchronization checkpoint: `p3.3a20a51-provider-pricing-daily-refresh-explicit-runtime-entry-wiring-v0443`
...
- Public tag `v0.4.41-alpha = <pending>`.
- Internal checkpoint included in the public Release: `p3.3a20a51-provider-pricing-daily-refresh-explicit-runtime-entry-wiring-v0443 = 1559545`.
- GitHub Release is non-draft and non-prerelease.
- GitHub Latest API returns `v0.4.41-alpha`.
- Release assets are exactly `bootstrap.sh` and `install.sh`.
- `cox --version` from the refreshed release line reports `public version: v0.4.41-alpha | <release-tag-commit>`.
...
- This documentation sync may advance `master` beyond the public Release commit. The public `v0.4.41-alpha` tag is created by this explicit Release-update task for the p3.3a20a51 explicit provider-owned daily-refresh runtime entry release commit.
...
`v0.4.41-alpha` currently covers these user-visible areas:
...
| --- | --- | --- | --- | --- | --- | --- |
| Release | `v0.4.41-alpha` current Latest | GitHub Latest Release exists with `isPrerelease=false`, assets exactly `bootstrap.sh` and `install.sh`, and no duplicate Release title in body. | `v0.4.41-alpha = <pending>` | Closed | 2026-06-08 | Includes the p2.19 custom provider registry, guided UI, Codex compatibility/wrapper hardening, status JSON, auxiliary-model leakage fix, profile-drift fail-closed guard, and real Codex entry validation. |
...
- `cox status reasoning --json` and `cox status --json reasoning` are the primary machine-readable reasoning-route examples; legacy `cox status thinking --json` and `cox status --json thinking` must remain accepted.
...
## Provider pricing resource profile

`_provider_pricing_resource_profile(provider_id)` exposes provider-owned pricing
metadata without switching runtime pricing behavior. DeepSeek currently reports
the only supported official pricing resource. Other providers return an explicit
unsupported profile without DeepSeek URLs, currency, parser, or cache ownership.
...
The DeepSeek provider pricing source now explicitly owns the legacy cache schema
fields `unit_legacy`, `primary_locale`, and `fallback_locale`. The generic
provider pricing resource profile projects those fields without changing cache
selection, cache writing, daily refresh, usage pricing, or WeClaw behavior.

Providers without audited pricing support continue to return `None` for all
pricing source and cache-schema metadata.

## Provider pricing cache metadata builder

`_build_provider_pricing_cache_metadata(provider_id, ...)` now builds cache
metadata from the audited provider pricing resource profile. Unsupported or
incomplete providers fail explicitly instead of inheriting DeepSeek metadata.
...
`_provider_pricing_cache_path_profile(provider_id)` describes the audited
pricing-cache path ownership for a provider without changing active runtime
routing. DeepSeek currently resolves through the legacy shared
`COX_PRICING_CACHE_PATH` contract.
...
## Provider pricing cache migration status

`_provider_pricing_cache_migration_status(...)` is a read-only diagnostic
classifier. Callers provide the proposed provider-scoped cache path explicitly;
the function does not invent a path or alter current runtime routing.

The classifier reports `none`, `legacy_only`, `provider_only`,
`both_identical`, `both_conflict`, or `explicit_pricing_path`. File equality
...
`_write_provider_pricing_cache_atomic(...)` is an unwired provider-owned
writer seam. It requires both an explicit provider ID and an explicit target
path. Its atomic file operation is cloned from the legacy writer, while its
metadata includes provider, adapter, family, cache scope, schema owner and
provider-scoped ownership fields.

The seam does not call the legacy writer and is not connected to refresh,
reader selection, daily refresh, usage pricing or WeClaw. Automatic dual
writes, migration and writer switching remain disabled.

## Pricing refresh writer selection profile

`_provider_pricing_refresh_writer_selection_profile(...)` describes three
candidate modes: `legacy_shared`, `provider_owned`, and `disabled`. The default
description remains `legacy_shared`.

This is a profile-only contract. It does not call either writer, create files,
change refresh routing, change reader routing, or alter the daily refresh
target. `provider_owned` requires an explicit provider path. Dual writing and
fallback writing are explicitly unsupported because two separate atomic file
replacements do not form one cross-file transaction.

## Explicit pricing refresh writer activation contract

`_provider_pricing_refresh_writer_activation_contract(...)` validates explicit
activation intent without executing it. Both `activate` and `mode` are required
keyword-only arguments. `provider_owned` additionally requires an explicit
provider cache path.

The contract does not inspect activation environment variables or CLI state,
call a writer, create files, or change refresh, reader, daily-refresh, usage,
or WeClaw routing. Explicit activation of `legacy_shared` is rejected because
legacy shared writing remains the unchanged default compatibility path. Runtime
activation, dual writing, fallback writing, and automatic reader switching
remain unavailable.

## Provider-owned pricing refresh single-write execution seam

`_provider_pricing_refresh_writer_single_write_execution(...)` is an explicit,
function-level execution seam. It requires `activate=True`,
`mode=provider_owned`, and an explicit provider cache path. The initial
implementation is limited to DeepSeek and injects exactly one provider-owned
writer callback.

The seam is not connected to the existing refresh wrapper, CLI, environment
variables, daily refresh, readers, usage pricing, or WeClaw. It never performs
legacy writing, dual writing, fallback writing, or automatic path inference.

## Provider-owned pricing refresh CLI candidate contract

`_pricing_refresh_provider_owned_cli_candidate_contract(...)` validates a
future explicit CLI opt-in without registering new parser options or changing
pricing dispatch.

The candidate requires the combination `--write-cache`, `--provider-owned`,
and an explicit `--provider-cache-path`. The existing `--cache-path` remains a
legacy-shared path and is rejected when provider-owned mode is requested.

This contract does not call the provider-owned execution seam, read
environment variables, infer a path, write files, create directories, perform
dual or fallback writes, or change daily refresh and pricing readers.

## Provider-owned pricing refresh CLI single-write execution dispatch

The `pricing refresh` parser retains the explicit `--provider-owned` and
`--provider-cache-path PATH` arguments. A candidate request is processed in two
ordered stages:

1. `_pricing_refresh_provider_owned_cli_candidate_contract(...)` validates the
   complete explicit argument set.
2. A valid DeepSeek candidate calls
   `_provider_pricing_refresh_writer_single_write_execution(...)` exactly once.

The candidate mapping supplies `activate`, `mode`, and `provider_path`. Existing
CLI values supply `model`, `source_url`, and `timeout`. Invalid candidates never
call the execution seam.

The CLI normalizes the outward execution metadata to:
...
The explicit provider path is the sole write target. Legacy writer invocation,
legacy-path reinterpretation, path inference, activation environment lookup,
dual writing and fallback writing remain prohibited. Requests without candidate
arguments continue through the unchanged legacy refresh path.

This stage does not modify the candidate contract, the application execution
seam, daily refresh, pricing readers, usage pricing context, or WeClaw.
