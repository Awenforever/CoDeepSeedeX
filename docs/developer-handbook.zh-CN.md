...
- 主分支：`master`
- 当前公开Release：`v0.4.41-alpha`
- 当前公开Release类型：GitHub Latest普通alpha Release，`isPrerelease=false`
- 当前公开Release提交：`release v0.4.41-alpha`
- GitHub Latest普通Release：`v0.4.41-alpha`
- GitHub Release标题：`CodeXchange v0.4.41-alpha`
...
- Release资产digest：
  ...
  - `install.sh` sha256：`867c7233d297059394be92c30804258e5656319b9ab7695c669f0eab1b7cd0eb`
- 当前内部开发检查点：`p3.3a20a51-provider-pricing-daily-refresh-explicit-runtime-entry-wiring-v0443`
- 当前公开Release包含的最新运行时检查点：`p3.3a20a51-provider-pricing-daily-refresh-explicit-runtime-entry-wiring-v0443`
...
- 当前公开Release note同步检查点：`p3.3a20a51-provider-pricing-daily-refresh-explicit-runtime-entry-wiring-v0443`
- WeClaw要求：如果使用WeClaw集成，要求`weclaw_dev >= v0.1.9-alpha`。
- 未经明确Release更新任务不得移动的公开tag：
  - `v0.4.41-alpha = <pending>`
...
- 公开tag `v0.4.41-alpha = <pending>`。
- 当前公开Release包含的内部检查点：`p3.3a20a51-provider-pricing-daily-refresh-explicit-runtime-entry-wiring-v0443 = 1559545`。
- GitHub Release非draft、非prerelease。
- GitHub Latest API返回`v0.4.41-alpha`。
- Release资产只有`bootstrap.sh`和`install.sh`。
- 刷新后的Release线运行`cox --version`应报告`public version: v0.4.41-alpha | <release-tag-commit>`。
...
- 本次文档同步可能让`master`领先公开Release提交；公开`v0.4.41-alpha`标签由本次明确的Release更新任务创建，并指向p3.3a20a51显式provider-owned每日刷新运行时入口的发布提交。

## 3. 关键文件地图

- `codexchange_proxy/app.py`：运行时核心、Responses兼容接口、DeepSeek/custom provider桥接、工具桥接、provider分发、版本元数据、debug trace。
- `codexchange_proxy/cli.py`：`cox`命令入口、配置、custom provider registry、provider设置、post-config刷新、doctor命令、升级逻辑。
...
- `experiments/model-catalog/cox-proxy-models.json`：托管模型目录。
...
3. Codex兼容和wrapper可靠性
   ...
   - 生成的Codex wrapper必须解析真实Codex二进制，不能把`REAL_CODEX`指向另一个CodeXchange wrapper或`/tmp/codexchange-*`测试wrapper。
...
5. Provider bridges
   ...
   - Image generation默认验证不一定是真实生成；真实出图必须显式执行`cox doctor providers --live --allow-spend`。
...
12. **AnyCodeX未来命名边界。** CodeXchange仍是当前项目名和公开产品名。AnyCodeX只是未来方向，不得进入当前用户表面。
13. **完整源码优先审计规则。** 补丁设计必须基于完整文件、完整函数、完整模块或完整章节上下文，不能只靠grep/rg片段。
14. **不要迎合旧测试。** 如果测试断言过时行为或幽灵文档，应修改测试契约，而不是保留死代码或死文档。

### 7.2 Release专项规则

- 不要猜运行时版本文件路径。运行时文件是`codexchange_proxy/app.py`。
...
凡是可能影响用户安装态的开发任务，都必须在设计阶段同时检查install、upgrade和uninstall。至少判断是否涉及一键bootstrap、`scripts/install.sh`、`cox upgrade`、生成的`cox`或`codex`wrapper、`~/.codex/config.toml`、本地env文件、manifest-backed rollback、source archive fallback和Release资产。
...
未知用户自有`codex`或`cox`不能静默覆盖。已知CodeXchange wrapper可以备份后刷新。
...
- 运行时兼容通过legacy env mirror保持：`COX_MODEL_PROVIDER=custom`、`COX_MODEL_BASE_URL`、`COX_MODEL`和`COX_MODEL_API_KEY`。
...
不要新增用户未要求的`cox config test-provider --kind web-search|image --provider <name>`命令。
...
- Codex CLI `< 0.134.0`：使用`~/.codex/config.toml`里的legacy `[profiles.deepseek]`和`[profiles.cox]`表。
- Codex CLI `>= 0.134.0`：使用split profile files：`~/.codex/deepseek.config.toml`和`~/.codex/cox.config.toml`。

Wrapper契约：

- `REAL_CODEX`必须解析到真实Codex binary或Node入口，不能指向另一个CodeXchange wrapper。
- `/tmp/codexchange-*`路径不得被选为真实Codex。
...
git status --short
git rev-parse --short v0.4.41-alpha^{}
...
gh release view v0.4.41-alpha --repo Awenforever/CoDeepSeedeX --json tagName,name,isDraft,isPrerelease,targetCommitish,assets,publishedAt
...
v0.4.41-alpha=<release-commit>
p2.19a23-profile-drift-failclosed-guard=6a96593
GitHub Latest Release=v0.4.41-alpha
isDraft=false
isPrerelease=false
assets=[bootstrap.sh, install.sh]
bootstrap.sh sha256=5005d628290b569780c042e30ac7e44aff02b4df4b957f2159c25b69356dd12a
install.sh sha256=867c7233d297059394be92c30804258e5656319b9ab7695c669f0eab1b7cd0eb
public version: v0.4.41-alpha | <release-commit>
...
卸载不得删除无关用户文件和非CodeXchange配置。

## 14. 长期主线任务清单

| ID | 主线任务 | 预期指标 | 当前版本 / 锚点 | 当前状态 | 最近更新 | 备注 |
| --- | --- | --- | --- | --- | --- | --- |
| Release | `v0.4.41-alpha`当前Latest | GitHub Latest Release存在，`isPrerelease=false`，资产只有`bootstrap.sh`和`install.sh`，Release正文不重复标题。 | `v0.4.41-alpha = <pending>` | 已闭合 | 2026-06-08 | 包含p2.19 custom provider registry、guided UI、Codex兼容/wrapper加固、status JSON、辅助模型泄漏修复、profile漂移fail-closed guard和真实Codex入口验证。 |
...
| WeClaw | 完整遥测基线 | WeClaw可消费cox拥有的profile、model、effort、context、usage、pricing、cost、balance、Details、tokenizer和compaction契约。 | `v0.3.9-alpha = 82a4428` | 已闭合 | 2026-05-24 | 使用WeClaw时要求`weclaw_dev >= v0.1.9-alpha`。 |
...
- 生成的CodeXchange Codex wrapper不得把另一个CodeXchange wrapper作为`REAL_CODEX`。
- `cox profile refresh-wrapper`只有在能从`COX_REAL_CODEX`、`PATH`或常见npm/nvm位置解析出非CodeXchange真实Codex可执行文件时，才允许从污染的manifest `REAL_CODEX`中恢复。
- 如果没有安全真实Codex可执行文件，wrapper refresh必须fail closed。
- `/tmp/codexchange-*`测试HOME wrapper不得作为真实用户wrapper manifest的真实Codex目标。


### Real-HOME profile model consistency

- `COX_MODEL`中的active upstream model对两个managed Codex profile均为权威来源，除非显式配置`COX_THINKING_MODEL`覆盖`cox`。
- `config set-model`、`set-api-key`、custom-provider激活和guided wizard不得让`deepseek`与`cox` split profile残留不同upstream model。
- managed Codex profile的provider name仍保持本地cox provider（`cox-proxy`和`cox-proxy`）；上游provider/base URL/model由cox env契约承载。
- real-HOME repair必须保持Codex 0.134+ split profile布局，不得重新引入legacy `[profiles.deepseek*]`表。


### Status JSON与辅助模型泄漏

- `cox status --json`是普通proxy status JSON的显式机器可读别名。
- `cox status reasoning --json`和`cox status --json reasoning`是主机器可读reasoning路由示例；legacy `cox status thinking --json`和`cox status --json thinking`必须继续兼容接受。
- `--weclaw-json`仍是独立的WeClaw-facing契约，不得与普通proxy status JSON混淆。
- 在`COX_FORCE_MODEL=1`下，agent-liveness judge等辅助调用必须跟随`COX_MODEL`，不得静默选择另一个provider model。
- status表面应暴露辅助调用实际选择的upstream model，使用户路径验证能在真实模型调用前捕获泄漏。


### Profile drift fail-closed guard

- 当`COX_FORCE_MODEL=1`时，`COX_MODEL`仍是CodeXchange-managed Codex profiles的权威model来源。
- `cox start`和`cox status`在继续前会基于默认env执行静默managed-profile预修复。
...
`_provider_pricing_resource_profile(provider_id)` 只暴露 provider 所拥有的
定价资源元数据，不切换运行时定价行为。当前只有 DeepSeek 提供受支持的
官方定价资源；其他 provider 返回明确的不支持档案，不携带 DeepSeek
地址、币种、解析器或缓存所有权信息。

本版本中的该边界仅用于诊断。Daily refresh、旧共享定价缓存、usage ledger
定价上下文、WeClaw 定价输出以及 DeepSeek 兼容 wrapper 均保持原有行为。

## Provider 定价资源元数据补全

DeepSeek provider 的定价源现已明确拥有旧缓存 schema 中的
`unit_legacy`、`primary_locale` 和 `fallback_locale` 字段。通用 provider
定价资源档案仅投影这些字段，不改变缓存选择、缓存写入、daily refresh、
usage 定价或 WeClaw 行为。

尚未完成定价审计的 provider 继续为全部定价源和缓存 schema 元数据返回
`None`。

## Provider 定价缓存元数据 builder

`_build_provider_pricing_cache_metadata(provider_id, ...)` 根据已审计的
provider 定价资源档案构造缓存元数据。未支持或元数据不完整的 provider
会明确失败，不会继承 DeepSeek 元数据。

`_build_deepseek_pricing_cache_metadata(...)` 保留旧 DeepSeek 合同。
`_write_pricing_cache_atomic(...)` 的现有签名、调用路由、缓存路径行为和
序列化 payload 保持不变，仅将元数据构造委托给 DeepSeek 兼容 builder。

## Provider 定价缓存路径档案

`_provider_pricing_cache_path_profile(provider_id)` 描述 provider 经审计的
定价缓存路径所有权，但不会改变当前运行时路由。DeepSeek 目前仍通过旧的
共享 `COX_PRICING_CACHE_PATH` 合同解析路径。

`_provider_pricing_cache_path(provider_id)` 会拒绝尚未建立经审计缓存路径
合同的 provider。`_deepseek_pricing_cache_path()` 是显式的 DeepSeek
兼容 wrapper。现有 reader、writer、刷新路由、配置优先级、usage 定价和
WeClaw 仍继续调用旧 `_pricing_cache_path()` 与 `_pricing_config_path()`。

## Provider 定价缓存迁移状态

`_provider_pricing_cache_migration_status(...)` 是纯只读诊断分类器。
调用者必须显式传入拟议的 provider-scoped 缓存路径；该函数不会自行决定
路径，也不会改变当前运行时路由。

分类状态包括 `none`、`legacy_only`、`provider_only`、
`both_identical`、`both_conflict` 和 `explicit_pricing_path`。
文件一致性使用原始字节 SHA-256，因此即使 JSON 语义相同，只要字节格式
不同也会保守地判定为冲突。该函数不会复制、移动、删除、写入、选择或启用
任何缓存。`_deepseek_pricing_cache_migration_status(...)` 是显式的
DeepSeek 兼容 wrapper。

## Provider-owned 定价缓存 writer

`_write_provider_pricing_cache_atomic(...)` 是尚未接入运行链路的
provider-owned writer seam，必须显式传入 provider ID 和目标路径。其原子
文件操作由旧 writer 的实现克隆而来，但 metadata 会写入 provider、
adapter、family、cache scope、schema owner 和 provider-scoped 所有权字段。

该 seam 不调用旧 writer，也未接入 refresh、reader 选择、daily refresh、
usage 定价或 WeClaw。自动双写、自动迁移和 writer 切换仍保持禁用。

## 定价 refresh writer 选择 profile

`_provider_pricing_refresh_writer_selection_profile(...)` 只描述
`legacy_shared`、`provider_owned` 和 `disabled` 三种候选模式，默认描述仍为
`legacy_shared`。

该合同仅用于 profile 展示，不调用任何 writer、不创建文件、不修改 refresh
路由、不修改 reader 路由，也不改变 daily refresh 目标。`provider_owned`
必须显式提供 provider 路径。双写与 fallback write 明确不受支持，因为两个
独立的原子文件替换无法组成跨文件事务。

## 定价 refresh writer 显式激活合同

`_provider_pricing_refresh_writer_activation_contract(...)` 只校验显式激活
意图，不执行激活。`activate` 与 `mode` 都是必须显式传入的仅关键字参数；
`provider_owned` 还必须显式提供 provider 缓存路径。

该合同不读取激活环境变量或 CLI 状态，不调用 writer，不创建文件，也不修改
refresh、reader、daily refresh、usage 或 WeClaw 路由。`legacy_shared`
仍是兼容默认路径，因此禁止将其作为显式激活模式。运行时激活、双写、
fallback write 及自动 reader 切换继续不可用。

## Provider-owned 定价 refresh 单写执行 seam

`_provider_pricing_refresh_writer_single_write_execution(...)` 是显式的函数级
执行 seam。调用方必须传入 `activate=True`、`mode=provider_owned` 以及明确的
provider 缓存路径。初始实现仅支持 DeepSeek，并且只注入一个 provider-owned
writer callback。

该 seam 不接入现有 refresh wrapper、CLI、环境变量、daily refresh、reader、
usage 定价或 WeClaw；不会执行 legacy 写入、双写、fallback write 或自动路径
推断。

## Provider-owned 定价刷新 CLI 候选合同

`_pricing_refresh_provider_owned_cli_candidate_contract(...)` 仅验证未来显式
CLI opt-in 的参数组合，不注册新参数，也不改变当前 pricing dispatch。

候选组合必须同时包含 `--write-cache`、`--provider-owned` 和明确的
`--provider-cache-path`。现有 `--cache-path` 继续保持 legacy-shared 语义，
在 provider-owned 模式中必须拒绝。

该合同不会调用 provider-owned execution seam，不读取环境变量，不推断路径，
不写文件或创建目录，不执行双写或 fallback，也不改变 daily refresh 和定价
reader。

## Provider-owned 定价刷新 CLI 单写执行分派

`pricing refresh` parser 继续使用显式 `--provider-owned` 与
`--provider-cache-path PATH` 参数。candidate 请求按以下顺序处理：

1. `_pricing_refresh_provider_owned_cli_candidate_contract(...)` 验证完整的显式
   参数组合；
2. 合法的 DeepSeek candidate 仅调用一次
   `_provider_pricing_refresh_writer_single_write_execution(...)`。

candidate mapping 提供 `activate`、`mode` 与 `provider_path`，现有 CLI 参数提供
`model`、`source_url` 与 `timeout`。非法 candidate 绝不会调用 execution seam。

CLI 对外输出会归一化为：

- `execution.execution_seam=cli_explicit_provider_owned_dispatch`
- `execution.existing_cli_modified=true`
- `cli_execution.provider_execution_dispatch_wired=true`
- `cli_execution.execution_called=true`
- `cli_execution.candidate_validation_only=false`

显式 provider 路径是唯一写入目标。legacy writer 调用、legacy 路径重解释、路径
推断、环境变量激活、双写与 fallback 仍被禁止。未携带 candidate 参数的请求继续
进入完全不变的 legacy refresh 路径。

本阶段不修改 candidate contract、应用层 execution seam、daily refresh、定价
reader、usage pricing context 或 WeClaw。
