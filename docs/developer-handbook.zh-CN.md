...
- 当前内部开发检查点：`p2.12a5-token-compact-status-semantics`
- 最新闭合文档同步检查点：`p2.12a5-token-compact-status-semantics`
...
p2.12a5修正token-first Compact可观测状态语义。`tokens_to_auto_compact`始终是非负倒计时；超过阈值时通过`tokens_over_auto_compact_threshold`和`threshold_exceeded=true`表示。若已超过阈值但运行时不能修改payload，例如`too_few_messages`，可见状态为`skipped`，而不是`triggered`。ratio说明必须使用当前托管ratio，char运行时配置只能保留在诊断性的`legacy_char_debug`中，不能出现在主token上下文字段里。
