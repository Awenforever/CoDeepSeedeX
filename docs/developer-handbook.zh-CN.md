...
- 主分支：`master`
- 当前公开Release：`v0.4.2-alpha`
- 当前公开Release类型：pre-release
- 当前公开Release提交：发布后由`v0.4.2-alpha` tag解析
- GitHub Latest普通Release：`v0.4.0-alpha`
- GitHub Release标题：`CoDeepSeedeX v0.4.2-alpha`
- GitHub Release状态：非draft，非pre-release
- GitHub Release标志：`isDraft=false`，`isPrerelease=true`
- Release资产：`bootstrap.sh`，`install.sh`
- 当前内部开发检查点：`p2.17a7-custom-reasoning-output-diagnostics`
- 最新闭合文档同步检查点：`p2.17a7-custom-reasoning-output-diagnostics`
- 当前公开Release note同步检查点：`p2.17a7-custom-reasoning-output-diagnostics`
...
p2.15a4将p2.15的Codex 0.134+ profile布局和custom provider修复发布为公开`v0.4.2-alpha` pre-release。

Release边界：

1. `v0.4.2-alpha`为非draft、pre-release。
