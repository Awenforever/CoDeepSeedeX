...
- 本次文档同步可能让`master`领先公开Release提交；公开`v0.4.14-alpha`标签由本次明确的Release更新任务刷新到p2.22闭合发布提交。
...
git status --short
git rev-parse --short v0.4.14-alpha^{}
...
gh release view v0.4.14-alpha --repo Awenforever/CoDeepSeedeX --json tagName,name,isDraft,isPrerelease,targetCommitish,assets,publishedAt
...
v0.4.14-alpha=6a96593
p2.19a23-profile-drift-failclosed-guard=6a96593
GitHub Latest Release=v0.4.14-alpha
isDraft=false
isPrerelease=false
assets=[bootstrap.sh, install.sh]
bootstrap.sh sha256=5005d628290b569780c042e30ac7e44aff02b4df4b957f2159c25b69356dd12a
install.sh sha256=5a108260e6ef6a5a748f8f5ad8932be58b8f4f6baf1881c10f62eca430b6af9e
public version: v0.4.14-alpha | 6a96593
...
```bash
curl -fsSL https://github.com/Awenforever/CoDeepSeedeX/releases/download/v0.4.14-alpha/bootstrap.sh | bash -s -- --install-ref v0.4.14-alpha
```
...
| --- | --- | --- | --- | --- | --- | --- |
| Release | `v0.4.14-alpha`当前Latest | GitHub Latest Release存在，`isPrerelease=false`，资产只有`bootstrap.sh`和`install.sh`，Release正文不重复标题。 | `v0.4.14-alpha = <pending>` | 已闭合 | 2026-06-08 | 包含p2.19 custom provider registry、guided UI、Codex兼容/wrapper加固、status JSON、辅助模型泄漏修复、profile漂移fail-closed guard和真实Codex入口验证。 |
