...
COX_PUBLIC_RELEASE_TAG="${COX_LATEST_RELEASE_FALLBACK_TAG:-v0.4.41-alpha}"
...
show_install_completion_hold() {
  ...
  width="$(ui_terminal_width)"
  public_version="${COX_PUBLIC_VERSION:-v0.4.41-alpha}"
...
write_env_file() {
  ...
  if [ "$DRY_RUN" = "1" ]; then
    ...
    printf '+ atomically write private env data %q through shared lock\n' "$ENV_FILE" >> "$INSTALL_LOG"
    ok "Local env file written"
    return 0
  fi

  local env_python="$INSTALL_DIR/.venv/bin/python"
  local env_data_tool="$INSTALL_DIR/codexchange_proxy/env_file.py"
  if [ ! -x "$env_python" ] || [ ! -f "$env_data_tool" ]; then
  ...
  {
    printf 'COX_MODEL_API_KEY\0%s\0' "$final_api_key"
    printf 'COX_MODEL_BASE_URL\0%s\0' "$final_model_base_url"
    printf 'COX_MODEL_PROVIDER\0%s\0' "$final_model_provider"
    printf 'COX_CUSTOM_PROVIDER_NAME\0%s\0' "$final_custom_provider_name"
    printf 'COX_MODEL_PROVIDER_REGISTRY\0%s\0' "$MODEL_PROVIDER_REGISTRY_FILE"
    printf 'COX_INSTALL_DIR\0%s\0' "$INSTALL_DIR"
    printf 'COX_TOKENIZER_RESOURCE_DIR\0%s\0' "$INSTALL_DIR/resources/tokenizers"
    printf 'COX_PORT\0%s\0' "$stable_port"
    printf 'COX_THINKING_PORT\0%s\0' "$thinking_port"
    printf 'COX_MODEL\0%s\0' "$final_model_name"
    printf 'COX_REASONING_EFFORT\0%s\0' "max"
    printf 'COX_FORCE_MODEL\0%s\0' "1"
    if [ -n "${INSTALL_TARGET_INTERNAL_VERSION:-}" ]; then
      printf 'COX_INTERNAL_VERSION\0%s\0' "$INSTALL_TARGET_INTERNAL_VERSION"
    fi
    if [ -n "${INSTALL_TARGET_COMMIT:-}" ]; then
      printf 'COX_PUBLIC_COMMIT\0%s\0' "$INSTALL_TARGET_COMMIT"
      printf 'COX_INTERNAL_COMMIT\0%s\0' "$INSTALL_TARGET_COMMIT"
    fi
    printf 'COX_TOOL_MAX_ROUNDS\0%s\0' "6"
    printf 'COX_COMPACT_POLICY\0%s\0' "adaptive"
    printf 'COX_AGENT_LIVENESS_GUARD\0%s\0' "1"
    printf 'COX_AGENT_LIVENESS_JUDGE_ENABLED\0%s\0' "1"
    printf 'COX_AGENT_LIVENESS_JUDGE_MODEL\0%s\0' "v4-flash-no-thinking"
    printf 'COX_CODEX_TOOL_PROTOCOL_INSTRUCTION\0%s\0' "1"
    if [ -n "$final_web_search_key" ]; then
      printf 'COX_TOOL_BRIDGE\0%s\0' "1"
      printf 'COX_WEB_SEARCH_PROVIDER\0%s\0' "$final_web_search_provider"
      printf 'COX_WEB_SEARCH_MAX_RESULTS\0%s\0' "6"
      printf 'COX_WEB_SEARCH_TIMEOUT_SECONDS\0%s\0' "12.5"
      case "$final_web_search_provider" in
        tavily) printf 'TAVILY_API_KEY\0%s\0' "$final_web_search_key" ;;
        exa) printf 'EXA_API_KEY\0%s\0' "$final_web_search_key" ;;
        firecrawl) printf 'FIRECRAWL_API_KEY\0%s\0' "$final_web_search_key" ;;
        *) printf 'SERPAPI_API_KEY\0%s\0' "$final_web_search_key" ;;
      esac
    fi
    if [ -n "$final_image_api_key" ]; then
      printf 'COX_TOOL_BRIDGE\0%s\0' "1"
      printf 'COX_IMAGE_PROVIDER\0%s\0' "$final_image_provider"
      if [ "$final_image_provider" = "qwen_image" ] || [ "$final_image_provider" = "qwen_image_beijing" ] || [ "$final_image_provider" = "qwen_image_singapore" ]; then
        printf 'COX_IMAGE_MODEL\0%s\0' "qwen-image-2.0-pro"
      elif [ "$final_image_provider" = "stability" ]; then
        printf 'COX_IMAGE_MODEL\0%s\0' "stable-image-core"
      elif [ "$final_image_provider" = "fal" ]; then
        printf 'COX_IMAGE_MODEL\0%s\0' "fal-ai/flux/schnell"
      else
        printf 'COX_IMAGE_MODEL\0%s\0' "cogView-4-250304"
      fi
      printf 'COX_IMAGE_SIZE\0%s\0' "1024x1024"
      printf 'COX_IMAGE_N\0%s\0' "1"
      printf 'COX_IMAGE_DOWNLOAD\0%s\0' "1"
      printf 'COX_IMAGE_API_KEY\0%s\0' "$final_image_api_key"
    fi
  } | "$env_python" "$env_data_tool" replace-exports-nul "$ENV_FILE" \
      --header "# codexchange local environment" \
      --header "# Generated by scripts/install.sh"
...
ok "Proxy ports selected automatically: standard=$STABLE_PORT, reasoning=$THINKING_PORT"
