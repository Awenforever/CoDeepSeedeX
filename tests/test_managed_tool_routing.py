...
def test_native_web_search_maps_to_managed_tool_by_default(monkeypatch):
    monkeypatch.delenv("COX_WEB_SEARCH_ROUTING", raising=False)
...
def test_native_image_generation_maps_to_managed_tool_by_default(monkeypatch):
    monkeypatch.delenv("COX_IMAGE_GENERATION_ROUTING", raising=False)
    normalized, warnings, report = _normalize_with_report({"type": "image_generation"})

    assert normalized["type"] == "function"
    assert normalized["function"]["name"] == "codexchange_generate_image"
    assert any(item.get("mapped_to") == "codexchange_generate_image" for item in warnings)
    assert report["managed_tools_injected"] == ["codexchange_generate_image"]


def test_managed_tool_routing_policy_native_only_does_not_inject_managed_tool(monkeypatch):
    monkeypatch.setenv("COX_WEB_SEARCH_ROUTING", "native-only")
    normalized, warnings, report = _normalize_with_report({"type": "web_search"})
...
def test_managed_tool_routing_policy_disabled_drops_native_tool_with_action(monkeypatch):
    monkeypatch.setenv("COX_IMAGE_GENERATION_ROUTING", "disabled")
    normalized, warnings, report = _normalize_with_report({"type": "image_generation"})
...
async def test_managed_web_search_tool_executes_and_second_model_call_receives_result(monkeypatch, tmp_path):
    ...
    fake = FakeDeepSeekClient(
        [
            tool_call_response("call_1", "codexchange_web_search", {"query": "managed route", "max_results": 1}),
    ...
    assert "codexchange_web_search" in first_tool_names
    ...
    assert last_decision["managed_function_name"] == "codexchange_web_search"
    assert last_decision["action"] == "mapped_to_managed"
    last_execution = status_data["tool_bridge"]["web_search"]["last_execution"]
    assert last_execution["tool_name"] == "codexchange_web_search"
    ...
    assert any(item.get("mapped_to") == "codexchange_web_search" for item in routing)
...
async def test_managed_image_generation_tool_executes_and_surfaces_image_evidence(monkeypatch, tmp_path):
    ...
    fake = FakeDeepSeekClient(
        [
            tool_call_response("call_1", "codexchange_generate_image", {"prompt": "managed image", "n": 1}),
    ...
    assert last_execution["tool_name"] == "codexchange_generate_image"
    assert last_execution["ok"] is True
    assert last_execution["provider"] == "mock"
...
async def test_no_native_image_generation_tool_observed_is_explained(monkeypatch, tmp_path):
    ...
    assert image_status["managed_function_name"] == "codexchange_generate_image"
...
def test_tool_bridge_status_exposes_managed_tool_routing_registry(monkeypatch):
    ...
    assert routing["capabilities"]["web_search"]["managed_function_name"] == "codexchange_web_search"
    assert routing["capabilities"]["image_generation"]["managed_function_name"] == "codexchange_generate_image"
    assert data["tool_bridge"]["web_search"]["routing_policy"] == "auto"
    assert data["tool_bridge"]["image_generation"]["routing_policy"] == "auto"


_MANAGED_MARKER = "[codexchange managed tool routing]"
