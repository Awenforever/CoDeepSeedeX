...
def test_zhipu_native_status_is_exposed_at_model_api_top_level() -> None:
    for provider, adapter_id in (("zhipu", "zhipu"), ("zhipu_coding", "zhipu_coding")):
        ...
        for command in ("show", "status"):
            ...
            assert model_api["adapter_matrix_summary"]["native_count"] == 6
            assert model_api["adapter_matrix_summary"]["generic_count"] == 4

        assert payloads["show"] == payloads["status"]


def test_provider_adapter_contract_documents_current_zhipu_native_matrix() -> None:
    text = Path("docs/provider-adapter-contract.md").read_text(encoding="utf-8")

    assert "The current adapter matrix after the Qwen and Zhipu native adapter skeletons is:" in text
    ...
    assert "| `zai` | generic | `openai_compatible` |" in text
...
def test_installer_provider_classification_matches_native_adapter_matrix() -> None:
    ...
    assert 'zhipu|zhipu-coding|qwen-beijing|qwen-singapore|qwen-us) printf \'%s\\n\' "Built-in native adapter" ;;' in text
