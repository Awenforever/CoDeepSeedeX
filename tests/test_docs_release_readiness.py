...
def test_developer_handbook_current_release_state_is_synced_to_latest_release_note_node() -> None:
    ...
    assert "Current public Release commit: `d674a61`" in en_current
    assert "Current internal development checkpoint: `p2.12a13-remove-tracked-release-notes`" in en_current
    assert "Latest closed documentation sync checkpoint: `p2.12a13-remove-tracked-release-notes`" in en_current
    assert "Current public Release note synchronization checkpoint: `p2.12a13-remove-tracked-release-notes`" in en_current
    assert "  - `v0.3.9-alpha = d674a61`" in en_current

    assert "当前公开Release：`v0.3.9-alpha`" in zh_current
    assert "当前公开Release提交：`d674a61`" in zh_current
    assert "当前内部开发检查点：`p2.12a13-remove-tracked-release-notes`" in zh_current
    assert "最新闭合文档同步检查点：`p2.12a13-remove-tracked-release-notes`" in zh_current
    assert "当前公开Release note同步检查点：`p2.12a13-remove-tracked-release-notes`" in zh_current
    assert "  - `v0.3.9-alpha = d674a61`" in zh_current
