...
def test_developer_handbook_current_release_state_is_synced_to_latest_release_note_node() -> None:
    ...
    assert "Current public Release: `v0.4.41-alpha`" in en_current
    assert "Current public Release kind: ordinary GitHub Latest alpha Release, with `isPrerelease=false`" in en_current
    assert "Current public Release commit: `release v0.4.41-alpha`" in en_current
    assert "GitHub Latest ordinary Release: `v0.4.41-alpha`" in en_current
    ...
    assert "`install.sh` sha256: `867c7233d297059394be92c30804258e5656319b9ab7695c669f0eab1b7cd0eb`" in en_current
    assert "Current internal development checkpoint: `p3.3a20a51-provider-pricing-daily-refresh-explicit-runtime-entry-wiring-v0443`" in en_current
    assert "Latest runtime checkpoint included in the public Release: `p3.3a20a51-provider-pricing-daily-refresh-explicit-runtime-entry-wiring-v0443`" in en_current
    ...
    assert "Current public Release note synchronization checkpoint: `p3.3a20a51-provider-pricing-daily-refresh-explicit-runtime-entry-wiring-v0443`" in en_current
    assert "The public `v0.4.41-alpha` tag is created by this explicit Release-update task for the p3.3a20a51 explicit provider-owned daily-refresh runtime entry release commit." in en_current
    assert "  - `v0.3.9-alpha = 82a4428`" in en_current

    assert "当前公开Release：`v0.4.41-alpha`" in zh_current
    assert "当前公开Release类型：GitHub Latest普通alpha Release，`isPrerelease=false`" in zh_current
    assert "当前公开Release提交：`release v0.4.41-alpha`" in zh_current
    assert "GitHub Latest普通Release：`v0.4.41-alpha`" in zh_current
    ...
    assert "`install.sh` sha256：`867c7233d297059394be92c30804258e5656319b9ab7695c669f0eab1b7cd0eb`" in zh_current
    assert "当前内部开发检查点：`p3.3a20a51-provider-pricing-daily-refresh-explicit-runtime-entry-wiring-v0443`" in zh_current
    assert "当前公开Release包含的最新运行时检查点：`p3.3a20a51-provider-pricing-daily-refresh-explicit-runtime-entry-wiring-v0443`" in zh_current
    ...
    assert "当前公开Release note同步检查点：`p3.3a20a51-provider-pricing-daily-refresh-explicit-runtime-entry-wiring-v0443`" in zh_current
    assert "公开`v0.4.41-alpha`标签由本次明确的Release更新任务创建，并指向p3.3a20a51显式provider-owned每日刷新运行时入口的发布提交。" in zh_current
    ...
    stale_markers = [
        ...
        "Current public Release commit: resolved from `v0.4.41-alpha` tag after publication",
        "GitHub Latest ordinary Release: `v0.4.0-" + "alpha`",
        "GitHub Release flags: `isDraft=false`, `isPrerelease=" + "true`",
        "  - `v0.4.41-alpha = resolved by release " + "tag`",
        "当前公开Release类型：" + "pre-release",
        "当前公开Release提交：发布后由`v0.4.41-alpha` tag解析",
