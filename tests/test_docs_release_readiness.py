...
def test_developer_handbook_current_release_state_is_synced_to_latest_release_note_node() -> None:
    ...
    assert "Current internal development checkpoint: `p2.11a2-semantic-payload-enabled-runtime-status`" in en_current
    assert "Latest closed documentation sync checkpoint: `p2.11a2-semantic-payload-enabled-runtime-status`" in en_current
    ...
    assert "当前内部开发检查点：`p2.11a2-semantic-payload-enabled-runtime-status`" in zh_current
    assert "最新闭合文档同步检查点：`p2.11a2-semantic-payload-enabled-runtime-status`" in zh_current
