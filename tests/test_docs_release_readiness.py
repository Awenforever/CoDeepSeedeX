...
def test_developer_handbook_current_release_state_is_synced_to_latest_release_note_node() -> None:
    ...
    assert "Current public Release: `v0.4.20-alpha`" in en_current
    ...
    assert "GitHub Latest ordinary Release: `v0.4.20-alpha`" in en_current
    ...
    assert "The public `v0.4.20-alpha` tag is being refreshed by this explicit Release-update task to the p2.22 closeout release commit." in en_current
    assert "  - `v0.3.9-alpha = 82a4428`" in en_current

    assert "当前公开Release：`v0.4.20-alpha`" in zh_current
    ...
    assert "GitHub Latest普通Release：`v0.4.20-alpha`" in zh_current
    ...
    assert "公开`v0.4.20-alpha`标签由本次明确的Release更新任务刷新到p2.22闭合发布提交。" in zh_current
    ...
    stale_markers = [
        ...
        "Current public Release commit: resolved from `v0.4.20-alpha` tag after publication",
        "GitHub Latest ordinary Release: `v0.4.0-" + "alpha`",
        "GitHub Release flags: `isDraft=false`, `isPrerelease=" + "true`",
        "  - `v0.4.20-alpha = resolved by release " + "tag`",
        "当前公开Release类型：" + "pre-release",
        "当前公开Release提交：发布后由`v0.4.20-alpha` tag解析",
