...
def test_developer_handbook_current_release_state_is_synced_to_latest_release_note_node() -> None:
    ...
    assert "Current internal development checkpoint: `p2.19a15-provider-alias-boundary`" in en_current
    ...
    assert "Latest closed ghost audit tool checkpoint: `p2.19a15-provider-alias-boundary`" in en_current
    ...
    assert "当前内部开发检查点：`p2.19a15-provider-alias-boundary`" in zh_current
    ...
    assert "最新闭合幽灵审计工具检查点：`p2.19a15-provider-alias-boundary`" in zh_current
