...
def test_cli_version(capsys):
    ...
    assert "v2.7a11-semantic-compaction-foundation" in out
...
def test_cli_doctor_allow_down_returns_zero(monkeypatch, tmp_path, capsys):
    ...
    data = json.loads(capsys.readouterr().out)
    assert data["proxy_version"].startswith("v2.7a11-semantic-compaction-foundation")
...
def test_cli_start_rejects_different_running_proxy_version(monkeypatch, tmp_path, capsys):
    ...
    assert data["expected_version"].startswith("v2.7a11-semantic-compaction-foundation")
