...
def test_p219a6_default_output_does_not_require_manual_followup_commands() -> None:
    ...
    assert "Start using CodeXchange:" in text
...
def test_readmes_document_product_uninstall_entrypoint_and_scope() -> None:
    ...
    assert "installer, not `cox uninstall`" in readme
    assert "安装器，不是`cox uninstall`" in readme_zh
    assert "bash ~/.local/share/codexchange/scripts/install.sh --uninstall" in combined
    assert "bash ~/.local/share/codexchange/scripts/install.sh --uninstall --remove-files" in combined
    assert "cox" in combined
    assert "restores the previous `codex` command backup" in readme
    assert "恢复旧`codex`命令" in readme_zh
    assert "removes the `cox` wrapper" in readme
    assert "移除CodeXchange安装的`cox` wrapper" in readme_zh
    assert "~/.local/share/codexchange" in combined
...
def test_install_repairs_codex_model_catalog_before_completion_hold() -> None:
    text = (ROOT / "scripts" / "install.sh").read_text(encoding="utf-8")
    marker = "codexchange_repair_codex_model_catalog_json_v2746a1"
...
def test_installer_passes_model_catalog_to_primary_codex_profile() -> None:
    ...
    assert 'run_quiet "Codex profile installed: deepseek" "$INSTALL_DIR/.venv/bin/cox" install-codex-profile' not in text
...
def test_developer_handbook_provider_handoff_exists() -> None:
    ...
    assert "cox doctor providers --live --allow-spend" in text
    assert "codexchange_proxy/app.py" in text
...
def test_installer_image_validation_is_live_generation_not_non_generation_probe() -> None:
    text = INSTALL_SH.read_text(encoding="utf-8")
    assert "PYCOX_INSTALL_LIVE_IMAGE_VALIDATION_P210A24" in text
    ...
    assert ("PYCOX_INSTALL_IMAGE_VALIDATION_" + "P28A3") not in text
    assert ("non-" + "generating validation") not in text

def test_installer_non_interactive_image_provider_defaults_to_zhipu() -> None:
    text = INSTALL_SH.read_text(encoding="utf-8")
    assert 'PROMPTED_IMAGE_PROVIDER="$(env_file_value COX_IMAGE_PROVIDER)"' in text
    assert 'PROMPTED_IMAGE_PROVIDER="${COX_IMAGE_PROVIDER:-zhipu}"' in text
    assert 'PROMPTED_IMAGE_PROVIDER="${COX_IMAGE_PROVIDER:-glm}"' not in text
...
def test_installer_non_interactive_preserves_existing_model_provider_env() -> None:
    ...
    assert 'PROMPTED_MODEL_PROVIDER="$(env_file_value COX_MODEL_PROVIDER)"' in model_prompt
    assert 'PROMPTED_MODEL_PROVIDER="${COX_MODEL_PROVIDER:-}"' in model_prompt
    assert model_prompt.index('PROMPTED_MODEL_PROVIDER="$(env_file_value COX_MODEL_PROVIDER)"') < model_prompt.index('PROMPTED_MODEL_PROVIDER="${COX_MODEL_PROVIDER:-}"')
    assert 'PROMPTED_MODEL_BASE_URL="$(env_file_value COX_MODEL_BASE_URL)"' in model_prompt
    assert 'PROMPTED_MODEL_BASE_URL="${COX_MODEL_BASE_URL:-}"' in model_prompt
    assert model_prompt.index('PROMPTED_MODEL_BASE_URL="$(env_file_value COX_MODEL_BASE_URL)"') < model_prompt.index('PROMPTED_MODEL_BASE_URL="${COX_MODEL_BASE_URL:-}"')
    assert 'PROMPTED_MODEL_NAME="$(env_file_value COX_MODEL)"' in model_prompt
    assert 'PROMPTED_MODEL_NAME="${COX_MODEL:-}"' in model_prompt
    assert model_prompt.index('PROMPTED_MODEL_NAME="$(env_file_value COX_MODEL)"') < model_prompt.index('PROMPTED_MODEL_NAME="${COX_MODEL:-}"')
...
def test_installer_non_interactive_prefers_target_env_file_over_ambient_shell_env() -> None:
    ...
    assert model_prompt.index('PROMPTED_API_KEY="$(env_file_value COX_MODEL_API_KEY)"') < model_prompt.index('PROMPTED_API_KEY="${COX_MODEL_API_KEY:-}"')

    web_prompt = text[text.index("prompt_serpapi_api_key() {"):text.index("local configure=", text.index("prompt_serpapi_api_key() {"))]
    assert 'PROMPTED_WEB_SEARCH_PROVIDER="$(env_file_value COX_WEB_SEARCH_PROVIDER)"' in web_prompt
    assert 'PROMPTED_WEB_SEARCH_PROVIDER="${COX_WEB_SEARCH_PROVIDER:-serpapi}"' in web_prompt
    assert web_prompt.index('PROMPTED_WEB_SEARCH_PROVIDER="$(env_file_value COX_WEB_SEARCH_PROVIDER)"') < web_prompt.index('PROMPTED_WEB_SEARCH_PROVIDER="${COX_WEB_SEARCH_PROVIDER:-serpapi}"')
    ...
    assert 'PROMPTED_IMAGE_PROVIDER="$(env_file_value COX_IMAGE_PROVIDER)"' in image_prompt
    assert 'PROMPTED_IMAGE_PROVIDER="${COX_IMAGE_PROVIDER:-zhipu}"' in image_prompt
    assert image_prompt.index('PROMPTED_IMAGE_PROVIDER="$(env_file_value COX_IMAGE_PROVIDER)"') < image_prompt.index('PROMPTED_IMAGE_PROVIDER="${COX_IMAGE_PROVIDER:-zhipu}"')
    assert 'PROMPTED_IMAGE_API_KEY="$(env_file_value COX_IMAGE_API_KEY)"' in image_prompt
...
def test_installer_cox_overwrite_gate_blocks_wrapper_write() -> None:
    ...
    gate = body.index('require_safe_local_bin_overwrite "$BIN_DIR/cox" "cox command wrapper" "cox" "$FORCE_COX_WRAPPER" || return 1')
    write = body.index('cat > "$BIN_DIR/cox" <<EOF')
    assert gate < write


def test_installer_gates_unknown_local_bin_overwrites() -> None:
    text = INSTALL_SH.read_text(encoding="utf-8")
    assert 'FORCE_CODEX_WRAPPER="${COX_FORCE_CODEX_WRAPPER:-0}"' in text
    assert 'FORCE_COX_WRAPPER="${COX_FORCE_COX_WRAPPER:-0}"' in text
    assert "is_codexchange_managed_local_bin()" in text
    ...
    assert 'require_safe_local_bin_overwrite "$BIN_DIR/cox" "cox command wrapper" "cox" "$FORCE_COX_WRAPPER" || return 1' in text
    assert "Refusing to overwrite unknown existing $label in non-interactive mode" in text
    assert "COX_FORCE_CODEX_WRAPPER=1" in text
    assert "COX_FORCE_COX_WRAPPER=1" in text
...
def test_installer_recognizes_only_codexchange_managed_local_bins() -> None:
    text = INSTALL_SH.read_text(encoding="utf-8")
    assert "CodeXchange codex wrapper|COX_COMMAND|codexchange|start_cox_profile" in text
    assert "CodeXchange|codexchange|\\.venv/bin/cox" in text
...
def test_installer_backs_up_local_files_before_refreshing_wrappers_and_config() -> None:
    text = INSTALL_SH.read_text(encoding="utf-8")
    assert 'LOCAL_BACKUP_DIR="${COX_BACKUP_DIR:-/tmp/codexchange-install-backups-$(date +%Y%m%d_%H%M%S)}"' in text
    assert "backup_local_file_before_overwrite()" in text
    assert 'backup_local_file_before_overwrite "$ENV_FILE" "local env file"' in text
    assert 'require_safe_local_bin_overwrite "$BIN_DIR/cox" "cox command wrapper" "cox" "$FORCE_COX_WRAPPER" || return 1' in text
    assert 'require_safe_local_bin_overwrite "$wrapper_path" "codex command wrapper" "codex" "$FORCE_CODEX_WRAPPER" || return 1' in text
    assert 'backup_local_file_before_overwrite "$HOME/.codex/config.toml" "Codex main config"' in text
    assert 'write_cox_wrapper' in text
...
def test_installer_p210a15_provider_flow_and_archive_fallback() -> None:
    text = INSTALL_SH.read_text(encoding="utf-8")
    assert "printf \'  CodeXchange \\033[1;35m%s\\033[0m\\n\' \"${INSTALL_REF:-GitHub Latest}\"" in text
    assert '"Y|$yes_label|plain"' in text
    assert '"Y|$yes_label|supported"' not in text
...
def test_installer_logo_function_renders_without_backtick_substitution(tmp_path) -> None:
    text = INSTALL_SH.read_text(encoding="utf-8")
    assert "cat <<'COX_INSTALLER_LOGO_ART'" in text
    assert "printf '  CodeXchange \\033[1;35m%s\\033[0m\\n' \"${INSTALL_REF:-GitHub Latest}\"" in text
...
def test_installer_logo_colors_version() -> None:
    text = INSTALL_SH.read_text(encoding="utf-8")
    assert "CodeXchange \\033[1;35m%s\\033[0m" in text
...
def test_installer_secret_prompt_keeps_existing_key_without_counting_it_as_new_input() -> None:
    ...
    assert "__COX_KEEP_EXISTING__" in text
...
def test_installer_codex_wrapper_prompt_explains_profile_usage() -> None:
    text = INSTALL_SH.read_text(encoding="utf-8")
    assert "After installing, use codex --profile cox." in text
    assert "Custom providers can use codex --profile <provider-id>." in text
    assert "The wrapper starts or refreshes the local cox backend automatically." in text
...
def test_installer_menu_prints_detail_between_prompt_and_global_help() -> None:
    ...
    assert "COX_NEXT_MENU_DETAIL" in text
    assert 'local detail="${COX_NEXT_MENU_DETAIL:-}"' in text
...
def test_installer_prints_combined_install_logs() -> None:
    text = INSTALL_SH.read_text(encoding="utf-8")
    assert 'BOOTSTRAP_LOG="${COX_BOOTSTRAP_LOG:-}"' in text
...
def test_legacy_profile_markers_are_absent_from_production_sources() -> None:
    ...
    """
    paths = [
        ROOT / "codexchange_proxy" / "app.py",
        ROOT / "codexchange_proxy" / "cli.py",
        ROOT / "scripts" / "cox-config",
        ROOT / "scripts" / "install.sh",
        ROOT / "README.md",
        ROOT / "README.zh-CN.md",
    ]
    pattern = re.compile(r"\[profiles\.(deepseek|cox)\]|profile = \\\"deepseek")
    offenders = []
    for path in paths:
...
def test_terminal_ui_uses_boxed_install_and_wizard_surfaces() -> None:
    ...
    assert "ui_box_top \"CodeXchange\"" in install_text
...
def test_terminal_ui_uses_fixed_step_labels_instead_of_interactive_placeholder() -> None:
    ...
    assert "__COX_BACK__" in install_text
    assert "Step 2/5" in cli_text
...
def test_installer_backspace_returns_previous_step_sentinel() -> None:
    text = INSTALL_SH.read_text(encoding="utf-8")
    assert "printf '%s\\n' \"__COX_BACK__\"" in text
    assert 'WRAPPER_CHOICE" = "__COX_BACK__"' in text
...
def test_installer_latest_release_api_falls_back_to_packaged_public_tag() -> None:
    text = INSTALL_SH.read_text(encoding="utf-8")
    assert 'COX_PUBLIC_RELEASE_TAG="${COX_LATEST_RELEASE_FALLBACK_TAG:-v0.4.10-alpha}"' in text
...
def test_installer_custom_provider_text_inputs_use_guided_panel() -> None:
    ...
    assert 'COX_INPUT_TITLE="$(model_api_provider_display_label) · Base URL"' in text
    assert 'COX_INPUT_TITLE="$(model_api_provider_display_label) · Model name"' in text
    assert 'COX_INPUT_TITLE="$(model_api_provider_display_label) · API key"' in text
    ...
    assert "Base URL is sent to cox/upstream" in text
    assert "Model name is sent to cox/upstream and must exactly match this provider" in text
...
def test_p218a3_stable_input_contract() -> None:
    ...
    assert 'printf \'%s\\n\' "__COX_BACK__"' in install_text

    # Base URL and model fields now use a value handoff so each field can return
    # __COX_BACK__ before mutating the persisted prompt variables.
    assert 'value="$(read_from_tty "Base URL"' in install_text
    assert 'if [ "$value" = "__COX_BACK__" ]; then' in install_text
...
def test_p218a4_installer_has_stable_splash_before_language_step() -> None:
    ...
    assert "COX_START_SETUP" in install_text
    ...
    start_setup_idx = install_text.index("COX_START_SETUP", splash_idx)
...
def test_p218a6_installer_has_completion_hold() -> None:
    ...
    assert "COX_FINISH_SETUP" in install_text
    assert "cox config show" in install_text
    assert "codex --profile cox" in install_text
...
def test_p218a9_model_api_panels_are_concise_and_validation_hold() -> None:
    ...
    assert "COX_MODEL_API_VALIDATION_CONTINUE" in install_text
    assert "show_model_api_validation_hold\n        return 0" in install_text

    assert 'COX_INPUT_TITLE="$(model_api_provider_display_label) · Base URL"' in install_text
    assert 'value="$(read_from_tty "Base URL"' in install_text
    assert 'COX_INPUT_TITLE="$(model_api_provider_display_label) · Model name"' in install_text
    assert 'value="$(read_from_tty "Model name"' in install_text
    assert 'COX_INPUT_TITLE="$(model_api_provider_display_label) · API key"' in install_text
    assert 'candidate="$(read_secret_from_tty "API key"' in install_text

    assert 'COX_INPUT_TITLE="Model API key"' not in install_text
...
def test_p219a3_custom_provider_name_has_no_site_specific_default() -> None:
    ...
    assert "Custom Provider · Provider name" in text
    assert "Provider name is only for your display and switching; it is not sent upstream." in text
    assert 'value="$(read_from_tty "Provider name" "${PROMPTED_CUSTOM_PROVIDER_NAME:-${COX_CUSTOM_PROVIDER_NAME:-}}")"' in text
    assert 'value="$(read_from_tty "Provider name" "${PROMPTED_CUSTOM_PROVIDER_NAME:-${COX_CUSTOM_PROVIDER_NAME:-USTC}}")"' not in text
...
def test_p219a6_completion_page_uses_start_using_and_optional_verification() -> None:
    ...
    assert "Start using CodeXchange:" in func
    assert "Optional verification:" in func
    assert "codex --profile cox" in func
    assert "cox --version" in func
    assert "cox config show" in func
...
def test_p219a8_installer_skips_managed_wrappers_when_resolving_real_codex() -> None:
    ...
    assert "is_codexchange_codex_wrapper_candidate()" in text
    assert "is_valid_real_codex_candidate()" in text
    assert "COX_REAL_CODEX is not a valid real Codex binary" in text
    assert "/tmp/codexchange-*/.local/bin/codex" in text
    assert "CodeXchange codex wrapper|COX_COMMAND|start_cox_profile|codexchange" in text

    # p2.21a4: missing real Codex remains a clear diagnostic, but it is no
    # longer fatal for the rest of installation. CodeXchange must not install
    ...
    assert "CodeXchange does not install or patch Node automatically" in text
...
def test_p221a1_installer_persists_wrapper_path_and_diagnoses_entrypoints() -> None:
    ...
    assert "Current shell does not resolve cox to the CodeXchange wrapper" in text
...
def test_p221a1_config_wizard_collects_custom_provider_name_and_profile() -> None:
    ...
    assert "COX_CUSTOM_PROVIDER_NAME" in cli_text
    assert "COX_MODEL_PROVIDER_REGISTRY" in cli_text
...
def test_p221a2_installer_selects_compatible_python_before_requirements() -> None:
    ...
    assert "select_codexchange_python_bin()" in text
    assert "ensure_codexchange_python_bin" in text
    assert "python3.13" in text
    assert "python3.12" in text
    assert "python3.11" in text
    assert '"$INSTALL_DIR/.venv/bin/python"' in text
    assert "ensure_codexchange_python_bin\nPY_VERSION=" in text
    assert "CodeXchange does not install or patch Python automatically" in text
...
def test_p221a3_installer_selects_python_before_env_backed_guided_setup() -> None:
    ...
    assert 'if [ -n "${PYTHON_BIN:-}" ] && [ -n "${COX_SELECTED_PYTHON_VERSION:-}" ]; then' in text
    assert "return 0\n  fi\n  select_codexchange_python_bin" in text
    ...
    assert text.count("ensure_codexchange_python_bin\nchoose_installer_language") == 1
    ...
    main_flow_markers = [
        ': > "$INSTALL_LOG"',
        'COX_NEXT_MENU_DETAIL="Setup plan:',
    ...
    assert 'choose_installer_language() {' in text
    assert 'existing="$(env_file_value COX_LOCALE)"' in text
    assert 'DEFAULT_STABLE_PORT="$(env_file_value COX_PORT)"' in text
    assert 'DEFAULT_THINKING_PORT="$(env_file_value COX_THINKING_PORT)"' in text
