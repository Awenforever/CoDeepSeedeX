...
TOOLS = [
    {
        "name": "safe_tool",
        "description": "A fake readonly tool that returns isError.",
        "inputSchema": {"type": "object", "properties": {"q": {"type": "string"}}}
    }
]
    ...
    if method == "initialize":
        print(json.dumps({
            ...
            "result": {
                "protocolVersion": msg.get("params", {}).get("protocolVersion"),
                "capabilities": {"tools": {"listChanged": False}},
                "serverInfo": {"name": "fake-iserror-backend", "version": "0.0.1"}
            }
        }), flush=True)
    elif method == "notifications/initialized":
        continue
...
@pytest.mark.asyncio
async def test_stdio_backend_propagates_mcp_tool_result_error(tmp_path, monkeypatch):
    server = tmp_path / "fake_mcp_iserror_server.py"
    _write_fake_mcp_iserror_server(server)
...
''',
    ...
    fake = FakeDeepSeekClient(
        [
            ...
            _response({"role": "assistant", "content": "Tool-level error handled."}),
        ]
    )
    ...
    assert deepseek_response["choices"][0]["message"]["content"] == "Tool-level error handled."

    tool_payloads = _payloads_with_tool_messages(fake.payloads)
    result = json.loads([m for m in tool_payloads[0]["messages"] if m.get("role") == "tool"][0]["content"])
    assert result["ok"] is False
    assert result["error"] == "mcp_tool_result_error"
    assert result["result"]["isError"] is True
    assert result["result"]["content"][0]["text"] == "tool-level failure"
